...
<script src="/js/databemfeia.js"></script>

<script>
    let idesdepesquisa = [2, 6, 18, 38, 37, 36, 27, 29, 1, 35, 13];

    // let idesdepesquisa = [38];
    let dados = JSON.parse(datinha);
    let dadosFiltrados = [];
    let idatual = ""
    let count = 0;
    ...
    mapaDeGente.forEach((value, key) => {
        mapaDeGente.set(key, limparArray(value));
    });

    function limparArray(harei) {
        var atual = "";
        var temporary = [];


        harei.forEach(element => {
            if (atual != element.identificador_original) {
                atual = element.identificador_original;
    ...
    function contarQuantosCadaUm() {
        ...
        mapaDeGente.forEach((value, key) => {
            ...
            value.forEach(element => {
                if (first) {
                    first = false;
                    caminhoDeste = element.identificador_original;
                } else {

                    caminhoDeste = caminhoDeste + "->" + element.identificador_original;
                }
            });
            caminhoDeste = caminhoDeste + ";";
            str = str + caminhoDeste;
...
*/
    /*
        caminho.forEach(element => {
            ...
            if (idatual != element.identificador_original) {
                idatual = element.identificador_original;
            }
            console.log(element);
        });
        */
    ...
    function dale() {
        ...
        //find id in screen
        //   document.getElementById("Resultado").innerHTML = dados.identificador_original;
    }
</script>
