...
@section('script')
<script src='/js/dados_escolhas_minigame.js'></script>
...
<script>
    ...
    function readJsonFile() {
        //galera do pc
        let IDS_PESQUISA = GALERA_DO_VR_UM;
/*
        let dia = DIA_WIN;
        let mes = MES_WIN;
...
*/
        ...
        let dia = DIA_VR;
        let mes = MES_VR;
        let hora_min = HORA_VR_1;
        let hora_max = HORA_VR_2;


        //galera
        //let IDS_PESQUISA = [2, 6, 18, 38, 37, 36, 27, 29, 1, 35, 13];

        let dados = JSON.parse(ESCOLHAS_MINIGAME);
        let dadosFiltrados = [];
        let idatual = ""
        let count = 0;
        ...
        mapaDeGente.forEach((value, key) => {
            ...
            value.forEach(element => {
                if (element.esta_certo == true)
                    a += " " + element.identificador_original + " " + element.botao_clicado + "\n";
            });
            a += "----- ERRADOS -----\n";
            value.forEach(element => {
                if (element.esta_certo != true)
